...
private:
    ...
    Texture2D mTexture;
    Texture2D mTextureLeft;
    Texture2D mTextureRight;
    bool mHasSeparateTextures;
    ...
    bool mForceAnimation = false; // Skip normal animation logic
...
public:
    void forceAnimate(float deltaTime); // Animate even when idle (for attacks)
    void resetForceAnimation() { mForceAnimation = false; }
    ...
    bool checkCollisionWithAI(Entity *ai) const;
    ...
    void setLeftRightTextures(const char *leftFilepath, const char *rightFilepath, 
        TextureType type = SINGLE, Vector2 spriteSheetDimensions = {1.0f, 1.0f})
    {
        mTextureLeft = LoadTexture(leftFilepath);
        mTextureRight = LoadTexture(rightFilepath);
        mHasSeparateTextures = true;
        mTextureType = type;
        if (type == ATLAS)
        {
    ...
    void setAnimationAtlas(std::map<Direction, std::vector<int>> animationAtlas);
    ...
    { 
        ...
        if (mTextureType == ATLAS && mAnimationAtlas.find(mDirection) != mAnimationAtlas.end())
        {
            mAnimationIndices = mAnimationAtlas.at(mDirection);
            mCurrentFrameIndex = 0; // Start from first frame on direction change
        }
    }
